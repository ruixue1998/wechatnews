...
def call_ai_for_html_translation(html_content_snippet):
    ...
    It's specifically designed to preserve the tags and their attributes.
    """
...
# --- UPDATED FUNCTION: Now also adds margin to eligible <p> tags ---
def process_and_style_tags(soup):
    """
    Wraps text in <span>, shrinks font size, and adds margins with advanced exclusion rules.
    Excludes:
    1. Any <p> tag with the class 'h3-p-pair'.
    2. Any <p> or <li> tag located inside the great-grandparent of a <p.h3-p-pair> tag.
    Applies:
    - `font-size: 80%` to all eligible <p> and <li> tags.
    - `margin-bottom: 5%` to all eligible <p> tags.
    """
    # Phase 1: Identify all "exclusion zones"
    exclusion_zones = set()
    ...
    # Phase 2: Process all target tags with exclusion logic
    tags_to_process = soup.find_all(['p', 'li'])
    count = 0
    
    for tag in tags_to_process:
        # Rule 1: Exclude the trigger tags themselves
        if tag.name == 'p' and tag.has_attr('class') and 'h3-p-pair' in tag['class']:
            continue

        # Rule 2: Check if the tag is inside any of the identified exclusion zones
        ...
        # If the tag is not excluded, proceed with modification
...
# --- Main Function ---
def get_full_page_and_save(url, output_dir, output_filename="ifanr_translated_interactive_page.html"):
    """
    ...
    try:
        ...
        print(f"错误：创建文件夹 '{output_dir}' 失败: {e}")
        sys.exit(1)
    ...
    try:
        ...
        # --- Steps 1 through 6 are unchanged ---
        ...
        js_code = """
        // Scroll-to-pair functionality (single-click)
        document.addEventListener('DOMContentLoaded', function() {
            ...
            pairedElements.forEach(element => {
                ...
                element.addEventListener('click', function(e) {
                    // This check prevents the scroll from firing if the element is also a language-toggle element.
                    // Since toggle is now dblclick, this is less of an issue, but good for safety.
                    if (e.target.closest('[ondblclick*="toggleLang"]')) {
                       // Do nothing on single-click for bilingual elements
                       return;
                    }
        ...
        // Language toggle functionality (double-click)
        ...
        # 12. 【逻辑不变】为前两个 <section> 之间的内容添加交互式翻译
        print("\n--- 开始对主要内容进行交互式翻译 ---")
...
if __name__ == '__main__':
    # 1. 动态地从 RSS feed 获取最新的“早报”文章 URL
    ...
    if target_url:
        ...
        # 定义保存目录
        save_directory = r'D:\Desktop\RSS_Output'
        
        # 使用获取到的 URL 继续执行主处理流程
        get_full_page_and_save(target_url, save_directory)
    else:
        # 如果没有获取到 URL，则打印信息并退出
